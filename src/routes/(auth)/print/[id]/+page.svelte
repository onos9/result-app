<script lang="ts">
  ...
  let result = results?.find(
    (res) => $configs?.term == res?.term && $configs.academicYear == res.academicYear
  );
</script>

<div data-theme="light" class="container mx-auto w-full h-screen bg-custom">
  <div class="flex-col w-a4 mx-auto min-w-screen flex overflow-hidden mt-5">
    <Header />
